...
로봇 툴의 동작 범위를 허용된 작업 공간 내로 제한하기 위해 cell을 정의해 보겠습니다. XY 평면은 5각형의 펜스 안으로 제한하고, Z축 범위는 0~3400mm 로 제한합시다.
...
3. 확장 속성에서 `space/cells/cell 0`을 선택합니다. `일반` 탭에서 활성화 항목을 `항상 on`으로 설정합니다. 아래 그림과 같이 설정되어 있어야 합니다.

   ![](../_assets/ch05_50_cell_gen.PNG)


4. `영역` 탭에서 우측의 `Start Input` 버튼을 클릭합니다. (버튼은 `End Input`으로 바뀝니다.) 이제 마우스 좌버튼으로 3D 뷰에서 펜스 5개의 모서리의 약간 안쪽 바닥을 차례로 클릭합니다. 작업 공간을 의미하는 연두색 다각형 평면이 표시됩니다.

   ![](../_assets/ch05_53_cell_area.PNG)


5. 5개의 포인트를 모두 클릭했다면, `End Input` 버튼을 클릭합니다. (버튼은 다시 `Start Input`으로 바뀝니다.) 만일 영역을 다시 선택하고 싶다면, `Start Input` 버튼을 눌러 이전 정점들을 모두 클리어하고 다시 시작할 수 있습니다.

   ![](../_assets/ch05_54_cell_area2.PNG)


6. 수치를 정밀하게 조정해야 한다면, 테이블 위젯에 직접 수치를 타이핑하면 됩니다. 저장 버튼을 눌러야 3D 뷰에 반영됩니다.

   ![](../_assets/ch05_56_cell_area_adjust.PNG)


7. 이제 뷰를 회전시켜서 옆에서 확인해 봅시다. default Z 범위 설정이 20 ~ -20 mm이기 때문에, 옆에서 보면 작업 공간은 로봇 바닥 높이로 납작하게 형성되어 있습니다.

   ![](../_assets/ch05_58_z.PNG)


8. 로봇 좌표계의 높이가 800mm이고, Z축 범위는 월드 좌표계 기준 0~3400mm이어야 하므로, 로봇 좌표계 기준으로 Zmin~Zmax는 -800~2600mm 범위로 설정해야 합니다. 값 입력 후, 확장 속성(HRSafeSpace)의 ![](../_assets/toolbar_save.PNG) 버튼을 클릭하면, 아래와 같이 설정이 완료됩니다.
...
2. 확장 속성에서 `space/cells/cell 1`을 선택합니다. `일반` 탭에서 활성화 항목을 `항상 on`으로 설정하고, 타입은 `보호 공간`으로 설정합니다. 아래 그림과 같이 설정되어 있어야 합니다.

   ![](../_assets/ch05_62_ps01.PNG)


3. `영역` 탭에서 Z max를 20으로 설정하고, 우측의 `Start Input` 버튼을 클릭합니다. (버튼은 `End Input`으로 바뀝니다.) 이제 마우스 좌버튼으로 3D 뷰에서 인간 작업자 주변의 바닥 4개의 지점을 차례로 클릭합니다. 보호 공간을 의미하는 빨간색 다각형 평면이 표시됩니다.

   ![](../_assets/ch05_62_ps05.PNG)


4. 4개의 포인트를 모두 클릭했다면, `End Input` 버튼을 클릭합니다. (버튼은 다시 `Start Input`으로 바뀝니다.) 만일 영역을 다시 선택하고 싶다면, `Start Input` 버튼을 눌러 이전 정점들을 모두 클리어하고 다시 시작할 수 있습니다.
...
1. 확장 속성에서 `space/robot`을 선택합니다. 링크 3, 링크 2에 대해 각기 아래와 같이 변경하고 ![](../_assets/toolbar_save.PNG) 버튼을 클릭하면, 두 링크의 위치에 각기 캡슐 영역이 나타납니다.
